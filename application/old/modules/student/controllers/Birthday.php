 ...
 * @Birthday            : Class
 * @class name      : Birthday
 * @description     : Manage student birthday.  
...
class Birthday extends MY_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('Birthday_Model', 'birthday', true);   
    ...
    * @Birthday            : Function
    * @function name   : index
    * @description     : Load "Birthday List" user interface                 
    *                       
    * @param           : null
    * @return          : null 
    * ********************************************************** */
    public function index($school_id = null) {
        ...
        if($this->session->userdata('role_id') != SUPER_ADMIN){            
            ...
            $this->data['classes'] = $this->birthday->get_list('classes', $condition, '', '', '', 'id', 'ASC');
        }
        
        $this->data['texts'] = $this->birthday->get_sms_list($school_id);
        
        $this->data['roles'] = $this->birthday->get_list('roles', array('status' => 1), '', '', '', 'id', 'ASC');
        ...
        $this->layout->view('birthday/index', $this->data);  
    }
    ...
    * @Birthday            : Function
    * @function name   : add
    * @description     : Load "Add new student Birthday" user interface                 
    *                    and process to store "student Birthday" into database 
    * @param           : null
    * @return          : null 
    * ********************************************************** */
    public function add() {
        ...
        if ($_POST) {          
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                $insert_id = $this->db->insert('birthday_smses', $data);
                ...
                if ($insert_id) {
                    ...
                    redirect('student/birthday/index/'.$data['school_id']);
                    
                } else {
                    error($this->lang->line('sms_send_failed'));
                    redirect('student/birthday/add');
                }
            } else {
                error($this->lang->line('sms_send_failed'));
        ...
        $this->data['texts'] = $this->birthday->get_sms_list();
        $this->data['roles'] = $this->birthday->get_list('roles', array('status' => 1), '', '', '', 'id', 'ASC');
        $this->data['schools'] = $this->schools;
        $this->data['add'] = TRUE;
    ...
    * ********************************************************** */
    private function _get_posted_sms_data() {
        ...
        $school = $this->birthday->get_school_by_id($data['school_id']);
        if(!$school->academic_year_id){
            error($this->lang->line('set_academic_year_for_school'));
            redirect('student/birthday/index');
        }
    ...
    * ********************************************************** */
    private function _send_sms($data) {
        ...
        $receivers = '';
        $users = $this->birthday->get_user_list($data['school_id'], $data['role_id'], $this->input->post('receiver_id'), $this->input->post('class_id'));
        ...
        $this->birthday->update('text_messages', array('receivers' => $receivers), array('school_id'=>$data['school_id'],'id' => $data['text_msg_id']));
