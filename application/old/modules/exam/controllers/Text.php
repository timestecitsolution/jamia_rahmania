 ...
 * @author          : Codetroopers Team 	_send_sms
...
class Text extends MY_Controller {
    ...
    * @return          : null 
    * ********************************************************** */
    private function _send_sms_old($data) {

        $school = $this->mark->get_school_by_id($data['school_id']);
        $students = $this->mark->get_student_list_by_class($data['school_id'], $data['exam_id'], $data['class_id'], $this->input->post('receiver_id'), $school->academic_year_id, $data['subject_id']);
        ...
        if (!empty($students)) {
            ...
            foreach ($students AS $obj) {
                ...
                if(!empty($data['result_type']))
                {
                    ...
                    if($data['result_type'] == 2 || $data['result_type'] == 3)
                    {
                            ...
                            if(!empty($data['subject_id']))
                            {
                                ...
                                $marks = $this->mark->get_marks_list_by_student_with_subject_type($data['school_id'], $data['exam_id'], $data['class_id'], $obj->student_id, $data['subject_id'],$school->academic_year_id, $data['result_type']);
                            }
                            else
                            {
                                $marks = $this->mark->get_marks_list_by_student_type($data['school_id'], $data['exam_id'], $data['class_id'], $obj->student_id, $school->academic_year_id, $data['result_type']);
                            }

                            foreach ($marks as $mark) 
                            {
                           
                                $message .= 'Sub: '.strtoupper($mark->subject) . ' Obtained Mark :' . $x .'-'. $mark->obtain_total_mark ;
                            }
                    }
                    else
                    {
                            ...
                            if(!empty($data['subject_id']))
                            {
                                $marks = $this->mark->get_marks_list_by_student_with_subject($data['school_id'], $data['exam_id'], $data['class_id'], $obj->student_id, $data['subject_id'],$school->academic_year_id);
                            }
                            else
                            {
                            ...
                            foreach ($marks as $mark) 
                            {
                                ...
                                $message .= 'Sub: '.strtoupper($mark->subject) . 'MCQ: ' . $mark->tutorial_obtain . ' + CQ: ' . $mark->written_obtain . ' = ' .$mark->obtain_total_mark;
                            }
                ...
                {
                    if (strpos($body, $receiver->name) !== false) {
                        $body = str_replace($receiver->name, $obj->student_name, $body);
                    }
                } 
        ...
        // var_dump($data);
        $school = $this->mark->get_school_by_id($data['school_id']);
        $students = $this->mark->get_student_list_by_class($data['school_id'], $data['exam_id'], $data['class_id'], $this->input->post('receiver_id'), $school->academic_year_id, $data['subject_id'], $data['section_id']);
        ...
        if (!empty($students)) {
            ...
            foreach ($students AS $obj) {
                ...
                if(!empty($data['result_type']))
                {
                    ...
                    if($data['result_type'] == 2 || $data['result_type'] == 3)
                    {
                            ...
                            foreach ($marks as $mark) 
                            {
                                ...
                                $message .= 'Sub: '.strtoupper($mark->subject) . ' Obtained Mark :' . $x .'-'. $mark->obtain_total_mark . '/' . $mark->exam_total_mark;
                            }
                    }
                    else
                    {
                            ...
                            foreach ($marks as $mark) 
                            {
                                ...
                                $message .= 'Sub: '.strtoupper($mark->subject) . ' Obtained Mark: MCQ- ' . $mark->tutorial_obtain . '/'. $mark->tutorial_mark .' + CQ- ' . $mark->written_obtain . '/'.$mark->written_mark.'= ' .$mark->obtain_total_mark .'/'. $mark->exam_total_mark;
                            }
                ...
                // $exam_attendances = $this->db->get_where("exam_attendances", array('school_id'=> $data['school_id'], 'class_id' => $data['class_id'], 'subject_id' => $data['subject_id'], 'exam_id' => $data['exam_id'], 'student_id' => $obj->student_id, 'academic_year_id' => $school->academic_year_id))->row();

                $attendance = get_exam_attendance($data['school_id'], $obj->student_id, $school->academic_year_id, $data['exam_id'], $data['class_id'], '', $data['subject_id']);

                if($attendance==0)
                {
                    $subject_name =  $this->db->get_where("subjects", array('school_id'=> $data['school_id'], 'class_id' => $data['class_id'], 'id' => $data['subject_id']))->row();
                    $body = "You were absent in the " .$subject_name->name. " of Model test. Please, contact with us as soon as possible";
                }
