...
class Absentsms extends MY_Controller {
    ...
    * ********************************************************** */
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($insert_id) {
                    ...
                    if($data['sms_gateway'] == 'bulksmsbd')
                    {
                        
                        $sms_set_info = $this->db->get_where('sms_settings', array( 'school_id'=>$data['school_id']))->row();
                        $api_key = $sms_set_info->bulksmsbd_api_key;
                        $senderid = '8809617642622';
                            
                        if($_POST['receiver_id'] == 0)
                        {
                            $school = $this->db->get_where('schools', array( 'id'=>$data['school_id']))->row(); 
                        
                            $sms_user_info = $this->ajax->get_student_list_absent($_POST['class_id'],$_POST['school_id'],$school->academic_year_id,'',$_POST['absent_date'],$_POST['section_id']);

                            foreach ($sms_user_info as $obj) {
                                $message = str_replace('[name]', $obj->name, $data['body']);
                                $message = str_replace('[date]', $_POST['absent_date'], $message);
                                $student = $this->db->get_where('students', array( 'id'=>$obj->id))->row(); 
                                $number = $student->phone;
                                $this->sms_send_bulksmsbd($api_key,$senderid,$number,$message);
                            }
                        } 
                        else
                        {
                            $sms_user_info = $this->db->get_where('guardians', array( 'id'=>$_POST['receiver_id']))->row();

                            $api_key = $sms_set_info->bulksmsbd_api_key;
                            $senderid = '8809617642622';
                            $student = $this->db->get_where('students', array( 'id'=>$_POST['receiver_id']))->row();
                            $message = str_replace('[name]', $student->name, $data['body']);
                            $message = str_replace('[date]', $_POST['absent_date'], $message);
                            $number = $sms_user_info->phone;
    ...
    private function sms_send_bulksmsbd($api_key = null, $senderid = null, $number= null, $message = null) {
