...
class Dashboard_Model extends MY_Model {
    ...
    public function get_birthday_sms($school_id = null)
    {
        
        $today = date('Y-m-d');
        $this->db->select('TM.*');
        $this->db->from('birthday_smses AS TM');    
    ...
    public function get_total_student($school_id = null) {
        ...
        if ($this->session->userdata('role_id') == STUDENT) {
            ...
            $school = $this->get_single('schools', array('id' => 1));
    ...
    public function get_total_expenditure_today($school_id = null) {
        $today = date('Y-m-d');
        $this->db->select('SUM(E.amount) AS total_expenditure');
        $this->db->from('expenditures AS E');
    ...
    public function get_total_expenditure_month($school_id = null) {
        $first_day_this_month = date('Y-m-01'); 
        $last_day_this_month  = date('Y-m-t');

        $this->db->select('SUM(E.amount) AS total_expenditure');
        $this->db->from('expenditures AS E');
        $this->db->where('DATE(E.date) >=', $first_day_this_month);
        $this->db->where('DATE(E.date) <=', $last_day_this_month);
        if ($school_id) {
            $this->db->where('E.school_id', $school_id);
        }
        return $this->db->get()->row()->total_expenditure;
    ...
    public function get_total_income_today($school_id = null) {
        $today = date('Y-m-d');
        
        $this->db->select('SUM(T.amount) AS total_income');
        $this->db->from('transactions AS T');
        $this->db->where('T.payment_date', $today);
    ...
    public function get_total_income_month($school_id = null) {
        $first_day_this_month = date('Y-m-01'); 
        $last_day_this_month  = date('Y-m-t');
        
        $this->db->select('SUM(T.amount) AS total_income');
        $this->db->from('transactions AS T');
        $this->db->where('DATE(T.payment_date) >=', $first_day_this_month);
        $this->db->where('DATE(T.payment_date) <=', $last_day_this_month);
    ...
    public function get_search_teacher($school_id, $keyword){
        ...
        $this->db->or_like('D.title', $keyword, 'after');
