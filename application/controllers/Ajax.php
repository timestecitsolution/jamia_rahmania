...
class Ajax extends My_Controller {
    ...
    public function get_user_by_role() {
        ...
        if ($role_id == SUPER_ADMIN) {
            ...
            if ($class_id) {
                $users = $this->ajax->get_student_list($class_id, $school_id, $school->academic_year_id,$subject_id,$section_id);
            } else {
    ...
    public function get_user_by_role_for_absent() {
        ...
        if ($role_id == SUPER_ADMIN) {
            ...
            if ($class_id) {
                ...
                $users = $this->ajax->get_student_list_absent($class_id, $school_id, $school->academic_year_id,$subject_id,$absent_date,$section_id);
                // $users = $this->ajax->get_student_list($class_id, $school_id, $school->academic_year_id,$subject_id);
            } else {
                $users = $this->ajax->get_list('students', array('status' => 1,'school_id'=>$school_id), '', '', '', 'id', 'ASC');
            }
        } else {
    ...
    /*     * **************Function get_user_by_month**********************************
     * @type            : Function
     * @function name   : get_user_by_month
     * @description     : this function use school id and month for get due sstudent list
     * @param           : null 
     * @return          : $str string value with user role tag list 
     * ********************************************************** */

    public function get_user_by_month()
    {
    ...
    public function get_user_by_role_birthday() {
        ...
        $users = array();
        if ($role_id == TEACHER) {
            $users = $this->db->query('SELECT * FROM teachers WHERE DATE_FORMAT(dob,"%m-%d") = DATE_FORMAT(NOW(),"%m-%d")')->result();
        } 
        elseif ($role_id == STUDENT) {
            $users = $this->db->query('SELECT * FROM students WHERE DATE_FORMAT(dob,"%m-%d") = DATE_FORMAT(NOW(),"%m-%d")')->result();
        } 
        else {
    ...
    public function get_roll_by_section()
    {
        $school_id = $this->input->post('school_id');
        $class_id = $this->input->post('class_id');
        $section_id = $this->input->post('section_id');
    ...
    public function get_grade_by_mark() {
        
         $school_id  = $this->input->post('school_id');
         $exam_total_mark  = $this->input->post('exam_total_mark');
         $obtain_total_mark  = $this->input->post('obtain_total_mark');


         $conver_mark = ($obtain_total_mark/$exam_total_mark) * 100;
         
        $salary_grades = $this->ajax->get_list('grades', array('status'=>1, 'school_id'=>$school_id), '','', '', 'id', 'ASC'); 
        ...
        if (!empty($salary_grades)) {
            foreach ($salary_grades as $obj) {   
                ...
                $str .= '<option value="' . $obj->id . '" ' . $selected . '>' . $obj->name.' ['.$obj->point .']'.' </option>';
    ...
    public function get_guardian_by_school() {
        ...
        if (!empty($guardinas)) {
            foreach ($guardinas as $obj) {   
                ...
                $str .= '<option value="' . $obj->id . '" ' . $selected . '>' . $obj->name . '-'. $obj->phone .' </option>';
    ...
    public function get_class_by_school() {
         ...
         $school_id  = $this->input->post('school_id');
         $class_id  = $this->input->post('class_id');
         $auto_class_id  = $this->input->post('auto_class_id');
        ...
        if (!empty($classes)) {
            foreach ($classes as $obj) {   
                ...
                if(!empty($auto_class_id))
                {
                    $selected = $auto_class_id == $obj->id ? $select : '';
                }
                $str .= '<option value="' . $obj->id . '" ' . $selected . '>' . $obj->name . '</option>';
     ...
     /*****************Function get_exam_by_school and academic year**********************************
     * @type            : Function
    ...
    public function get_exam_by_academic_year_id() {
        
         $school_id  = $this->input->post('school_id');
         $exam_id  = $this->input->post('exam_id');
         $academic_year_id  = $this->input->post('academic_year_id');
         
        $exams = $this->ajax->get_list('exams', array('status'=>1, 'school_id'=>$school_id, 'academic_year_id' => $academic_year_id), '','', '', 'id', 'ASC'); 
