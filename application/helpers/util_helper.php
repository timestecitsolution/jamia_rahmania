...
if (!function_exists('cash_book_validation')) {

    function cash_book_validation($posted_data)
    {
       
        $logged_in_id = 0;
        $ci = & get_instance();
        $ci->db->select('GS.*');
        $ci->db->from('global_setting AS GS');
        $ci->db->where('GS.id', 1);

        $info = $ci->db->get()->row();
        if(!empty($info))
        {
            if($posted_data['edit_id']==1)
            {
                $where = array(
                    'validation_type' => $posted_data['validation_type'],
                    'transaction_id' => $posted_data['transaction_id']
                );

                $update_data['transaction_amount'] = $posted_data['transaction_amount'];
                $ci->db->where($where)->update('cash_book_validation_table', $update_data);
            }
            else
            {
                $logged_in_id = $ci->session->userdata('id');
                if($info->cash_book_validation == 1)
                {
                    $posted_data['adjustment_amount'] = 0;
                ...
                else
                {
                    ...
                    $posted_data['submission_date'] = $posted_data['transaction_date'];
                }
                $posted_data['created_by'] = logged_in_user_id();
                $posted_data['created_at'] = date('Y-m-d H:i:s');
                $ci->db->insert('cash_book_validation_table', $posted_data);
            }
...
if (!function_exists('convert_to_arabic_letters')) {
    function convert_to_arabic_letters($text) {
        $english = [
            'a', 'b', 'c', 'd', 'e', 'f', 'g',
            'h', 'i', 'j', 'k', 'l', 'm', 'n',
            'o', 'p', 'q', 'r', 's', 't', 'u',
...
if (!function_exists('convert_bangla_to_arabic')) {
    function convert_bangla_to_arabic($text)
    {
        $map = [
            // স্বরবর্ণ
            'অ' => 'ا', 'আ' => 'آ', 'ই' => 'إ', 'ঈ' => 'ي',
            ...
            // ব্যঞ্জনবর্ণ
            'ক' => 'ك', 'খ' => 'خ', 'গ' => 'ج', 'ঘ' => 'غه', 'ঙ' => 'نغ',
            'চ' => 'چ', 'ছ' => 'ش', 'জ' => 'ز', 'ঝ' => 'جه', 'ঞ' => 'نج',
            'ট' => 'ط', 'ঠ' => 'طه', 'ড' => 'ض', 'ঢ' => 'ضه', 'ণ' => 'ن',
            'ত' => 'ت', 'থ' => 'ث', 'দ' => 'د', 'ধ' => 'ده', 'ন' => 'ن',
            'প' => 'پ', 'ফ' => 'ف', 'ব' => 'ب', 'ভ' => 'به', 'ম' => 'م',
            'য' => 'ي', 'র' => 'ر', 'ল' => 'ل', 'শ' => 'ش', 'ষ' => 'ش', 'স' => 'س', 'হ' => 'ه',
            'ড়' => 'ر', 'ঢ়' => 'ره', 'য়' => 'ى',
...
if (!function_exists('get_donation_category')) {

    function get_donation_category() {
        $ci = & get_instance();
        return array(
            'monthly' => $ci->lang->line('monthly'),
            'quarterly' => $ci->lang->line('quarterly'),
            'half_yearly' => $ci->lang->line('half_yearly'),
            'yearly' => $ci->lang->line('yearly'),
            'one_time' => $ci->lang->line('one_time'),
            'zakat' => $ci->lang->line('zakat'),
            'boarding' => $ci->lang->line('boarding_fund')
...
if (!function_exists('format_rank_suffix')) {
function format_rank_suffix($rank) {
    if (!$rank) {
        return '--';
    }
...
if (!function_exists('zakat_fund')) {

    function zakat_fund()
    {
       
        $ci = & get_instance();
        $ci->db->select('GS.zakat_fund');
        $ci->db->from('global_setting AS GS');
        $ci->db->where('GS.id', 1);
...
if (!function_exists('boarding_fund')) {

    function boarding_fund()
    {
       
        $ci = & get_instance();
        $ci->db->select('GS.boarding_fund');
        $ci->db->from('global_setting AS GS');
        $ci->db->where('GS.id', 1);
        return $ci->db->get()->row();
    }
}
if (!function_exists('general_fund')) {

    function general_fund()
    {
       
        $ci = & get_instance();
        $ci->db->select('GS.general_fund');
        $ci->db->from('global_setting AS GS');
        $ci->db->where('GS.id', 1);
        return $ci->db->get()->row();
    }
}
if (!function_exists('construction_fund')) {

    function construction_fund()
    {
       
        $ci = & get_instance();
        $ci->db->select('GS.construction_fund');
        $ci->db->from('global_setting AS GS');
        $ci->db->where('GS.id', 1);
        return $ci->db->get()->row();
    }
...
if (!function_exists('mfs_name')) {

    function get_mfs_getway() {
        $ci = & get_instance();

        $methods = array(
            
        'bkash' => $ci->lang->line('bkash'), 
        'rocket' => $ci->lang->line('rocket'), 
        'nagad' => $ci->lang->line('nagad'),
        'upay' => $ci->lang->line('upay'),
        'ucash' => $ci->lang->line('ucash'),
        'surecash' => $ci->lang->line('surecash'));
...
if (!function_exists('get_guardian_by_id')) {

    function get_guardian_by_id($id = null) {
        $ci = & get_instance();
        $ci->db->select('G.*');
...
function convert_to_bangla_digits($number) {
...
if (!function_exists('get_exam_total_mark')) {
    ...
    function get_exam_total_mark($school_id, $exam_id, $student_id, $academic_year_id, $class_id, $section_id = null) {
        ...
        $ci = & get_instance();
        $ci->db->select('COUNT(M.id) AS total_subject, SUM(G.point) AS total_point, SUM(M.exam_total_mark) AS exam_mark, SUM(M.obtain_total_mark) AS obtain_mark, G.id as grade_id, G.name as grade');
        $ci->db->from('marks AS M');
...
function get_student_position_in_term($school_id, $academic_year_id, $class_id, $student_id, $section_id = null, $exam_id) {
...
function get_subject_exam_wise_markt($school_id, $academic_year_id, $exam_id, $class_id, $subject_id, $section_id = null, $student_id = null)
{
        $ci = & get_instance();
...
function get_grade_by_mark_tabulation($total_mark,$obtain_mark,$school_id) {

    $conver_mark = ($obtain_mark/$total_mark) * 100;
    $ci = & get_instance();
    $ci->db->select('*');
    $ci->db->from('grades');
    $ci->db->where('status', 1);
    $ci->db->where('school_id', $school_id);
    $salary_grades = $ci->db->get()->result();
...
if (!function_exists('get_user_by_role')) {
    ...
    function get_user_by_role($role_id, $user_id, $academic_year_id = null) {
     ...
     function get_student_by_class($class_id = null, $school_id = null, $academic_year_id= null, $student_id = null) {

        $ci = & get_instance();
        $ci->db->select('E.roll_no,  S.*, Sec.name as section, C.name as class_name');
        $ci->db->from('enrollments AS E');        
        $ci->db->join('students AS S', 'S.id = E.student_id', 'left');
        $ci->db->join('classes AS C', 'C.id = E.class_id', 'left');
        $ci->db->join('sections AS Sec', 'Sec.id = E.section_id', 'left');
        ...
        $ci->db->where('E.school_id', $school_id);       
        $ci->db->where('S.status_type', 'regular');  
...
function get_class_by_id($school_id,$class_id) {
        $ci = & get_instance();
        $ci->db->select('cs.*');
        $ci->db->from('classes AS cs');  
        if($school_id){
            $ci->db->where('cs.school_id', $school_id);
        }
        if($class_id){
            $ci->db->where('cs.id', $class_id);
        }
...
function get_section_by_id($school_id,$class_id,$section_id) {
        $ci = & get_instance();
        $ci->db->select('cs.*');
        $ci->db->from('sections AS cs');  
        if($school_id){
            $ci->db->where('cs.school_id', $school_id);
        }
        if($class_id){
            $ci->db->where('cs.class_id', $class_id);
        }
        if($section_id){
            $ci->db->where('cs.id', $class_id);
        }
...
if (!function_exists('get_paid_status')) {
    ...
    function get_paid_status($status) {
        ...
        $data = array(
            ...
            'partially_paid' => $ci->lang->line('partial')
        );
        return $data[$status];
    }
...
function get_donation_cycle_label($cycle) {
    $year_suffix = substr($cycle, -2); 
    $code = substr($cycle, 0, -3); 

    $months = [
        'm1' => 'January', 'm2' => 'February', 'm3' => 'March',
        'm4' => 'April', 'm5' => 'May', 'm6' => 'June',
        'm7' => 'July', 'm8' => 'August', 'm9' => 'September',
    ...
    if (isset($months[$code])) {
        return $months[$code] . ' - ' . $full_year;
    } elseif (isset($quarters[$code])) {
        return '(' . $quarters[$code] . ') - ' . $full_year;
    } elseif (isset($halves[$code])) {
        return '(' . $halves[$code] . ') - ' . $full_year;
...
if (!function_exists('get_sms_gateways')) {
    ...
    function get_sms_gateways( $school_id ) {
        ...
        if(!empty($data)){
            ...
            if ($data->bulksmsbd_status) {
                $gateways['bulksmsbd'] = 'Bulk SMS BD';
            }
