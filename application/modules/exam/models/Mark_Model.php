...
class Mark_Model extends MY_Model {
    ...
    public function get_student_list($school_id = null, $exam_id = null, $class_id = null, $section_id = null, $subject_id = null, $academic_year_id = null){
        $subject_info = $this->db->get_where('subjects', array('id' => $subject_id))->row();
        ...
        if($subject_info->is_group==1)
        {
            $this->db->where('S.group', $subject_info->group_id); 
        }
    ...
    // public function get_student_list_mark_print($school_id = null, $exam_id = null, $class_id = null, $section_id = null, $subject_id = null, $academic_year_id = null) {
    //         $subject_info = $this->db->get_where('subjects', array('id' => $subject_id))->row();

    //         $rank_condition = "academic_year_id = {$academic_year_id} AND school_id = {$school_id} AND class_id = {$class_id}";
    //         if ($section_id) {
    //             $rank_condition .= " AND section_id = {$section_id}";
    //         }
    ...
    //             S.photo,  
    //             S.phone, 
    //             FR.total_obtain_mark,
    //             FIND_IN_SET(FR.total_obtain_mark, (
    ...
    //         $this->db->from('exam_schedules AS ES');
    //         $this->db->join('classes AS C', 'C.id = ES.class_id', 'left');
    //         $this->db->join('enrollments AS E', 'E.class_id = ES.class_id AND E.academic_year_id = '.$academic_year_id, 'left');
    //         $this->db->join('students AS S', 'S.id = E.student_id', 'left');
    //         $this->db->join('final_results AS FR', 'FR.student_id = S.id AND FR.academic_year_id = '.$academic_year_id.' AND FR.class_id = '.$class_id.($section_id ? ' AND FR.section_id = '.$section_id : ''), 'left');
...
public function get_student_list_mark_print($school_id = null, $exam_id = null, $class_id = null, $section_id = null, $subject_id = null, $academic_year_id = null) {
            ...
            $rank_subquery = 
                "(SELECT
                    student_id,
                    total_obtain_mark,
                    @rank := IF(@prev_mark = total_obtain_mark, @rank, @rank + 1) AS rank,
                    @prev_mark := total_obtain_mark
                FROM final_results, (SELECT @rank := 0, @prev_mark := NULL) vars
                WHERE $rank_condition
                ORDER BY total_obtain_mark DESC
                ) AS ranks";
            ...
            $this->db->join($rank_subquery, 'ranks.student_id = FR.student_id', 'left');
    ...
    public function get_student_list_by_class_old($school_id = null, $exam_id = null, $class_id = null, $receiver_id = null, $academic_year_id = null){
    ...
    public function get_exam_wise_subject($exam_id = null){
        $this->db->select('S.name as subject, exams.title as exam');
        $this->db->from('subjects AS S'); 
        $this->db->join('exam_schedules AS E' , 'E.subject_id = S.id', 'left');  
        $this->db->join('exams' , 'exams.id = E.exam_id', 'left');  
        if(!empty($exam_id)){
    ...
    public function get_student_list_by_class($school_id = null, $exam_id = null, $class_id = null, $receiver_id = null, $academic_year_id = null, $subject_id = null, $section_id = null){
        ...
        if(!empty($section_id))
        {
            $this->db->where('EA.section_id', $section_id);
        }
    ...
    public function get_mark_sms_list($school_id = null){
        ...
        $this->db->select('MS.*, S.school_name, R.name AS receiver_type, AY.session_year, C.name AS class_name,sub.name as subject_name, EX.title AS exam_name');
        $this->db->from('mark_smses AS MS');        
        $this->db->join('classes AS C', 'C.id = MS.class_id', 'left');
        $this->db->join('subjects AS sub', 'sub.id = MS.subject_id', 'left');
    ...
    public function get_marks_list_by_student($school_id, $exam_id, $class_id, $student_id, $academic_year_id){
        ...
        $this->db->select('M.exam_total_mark,M.obtain_total_mark, M.written_mark, M.written_obtain, M.tutorial_mark, M.tutorial_obtain, S.name AS subject');
    ...
    public function get_marks_list_by_student_with_subject($school_id, $exam_id, $class_id, $student_id, $subject_id, $academic_year_id){
    ...
    public function get_marks_list_by_student_type($school_id, $exam_id, $class_id, $student_id, $academic_year_id ,$result_type){

        if($result_type == 2)
        {
            $this->db->select('M.written_mark as exam_total_mark, M.written_obtain as obtain_total_mark, S.author AS subject');
        }
        else if($result_type == 3)
        {
           $this->db->select('M.tutorial_mark as exam_total_mark, M.tutorial_obtain as obtain_total_mark, S.author AS subject'); 
        }
        else
        {
            $this->db->select('M.exam_total_mark as exam_total_mark, M.obtain_total_mark as obtain_total_mark, S.author AS subject');
        }
        
        $this->db->select('M.exam_total_mark, M.obtain_total_mark, S.author AS subject');
        $this->db->from('marks AS M'); 
        $this->db->join('subjects AS S', 'S.id = M.subject_id', 'left');
        $this->db->where('M.school_id', $school_id);
        $this->db->where('M.exam_id', $exam_id);
    ...
    public function get_marks_list_by_student_with_subject_type($school_id, $exam_id, $class_id, $student_id, $subject_id, $academic_year_id ,$result_type){
