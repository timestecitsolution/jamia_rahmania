...
class Web extends CI_Controller {
    ...
    public function school(){
        ...
        $school = $this->web->get_single('schools', array('status' => 1, 'school_url'=>$school_url)); 
         ...
         $setting = $this->web->get_single('global_setting', array('status' => 1));
         $default_school_id = isset($setting->default_school) ? $setting->default_school : 0;
        //  $school = $this->web->get_single('schools', array('status' => 1, 'id' => $default_school_id));
        ...
        if($this->session->userdata('front_school_id')){ 
            ...
            $this->data['footer_pages'] = $this->web->get_list('pages', array('status' => 1, 'page_location'=>'footer', 'school_id'=>$this->session->userdata('front_school_id')));
            $this->data['header_pages'] = $this->web->get_list('pages', array('status' => 1, 'page_location'=>'header',  'school_id'=>$this->session->userdata('front_school_id')));
            $this->data['notices'] = $this->web->get_list('notices', array('status' => 1,  'school_id'=>$this->session->userdata('front_school_id')));
            $this->data['opening_hour'] = $this->web->get_single('opening_hours', array('status' => 1, 'school_id'=>$this->session->userdata('front_school_id')));
            $this->data['galleries'] = $this->web->get_list('galleries', array('status'=>1, 'school_id'=>$school_id, 'is_view_on_web'=>1), '', '', '', 'id', 'DESC');
            $this->data['gallery_images'] = $this->web->get_gallery_image_list($school_id);
            $this->data['muhtamim_message'] = $this->web->get_single('muhtamim_message', array('designation' => 'muhtamim'));
            $this->data['founder_director_message'] = $this->web->get_single('muhtamim_message', array('designation' => 'founder_director'));
            $this->data['guardian_message'] = $this->web->get_guardians_feedback($school_id);

            $this->data['groups'] = get_groups();
            $this->data['student_count'] = [];

            foreach($this->data['groups'] as $key => $value){
                $count = $this->web->get_group_wise_student_count($key);
                $this->data['student_count'][$key] = $count;
            }
    ...
    public function application_form() {
        ...
        $this->data['groups'] = get_groups();
        $this->data['school'] = $this->web->get_single('schools', array('status' => 1, 'id'=>$this->session->userdata('front_school_id')));
        $this->data['classes'] = $this->web->get_list('classes', $condition, '','', '', 'id', 'ASC');
        $this->layout->title('আবেদন ফর্ম | ' . SMS);
        $this->layout->view('web/admission-form', $this->data);
    }
    ...
    * ********************************************************** */
    ...
    public function video_galleries() {
        ...
        if($this->session->userdata('front_school_id')){           
            ...
            $school = $this->web->get_single('schools', array('status' => 1, 'id' => $default_school_id));    
            $this->data['groups'] = get_groups();
            $this->data['galleries'] = $this->web->get_list('galleries', array('status'=>1, 'school_id'=>$school->id, 'is_view_on_web'=>1), '', '', '', 'id', 'DESC');
            $this->data['video_gallery'] = $this->web->get_list('gallery_videos', array('status'=>1, 'school_id'=>$school->id), '', '', '', 'id', 'DESC');
            $this->data['list'] = TRUE;
            $this->layout->title($this->lang->line('media_gallery') . ' | ' . SMS);
            $this->layout->view('video_gallery', $this->data);
    ...
    * ********************************************************** */
    public function staff() {
        ...
        if($this->session->userdata('front_school_id')){           
            ...
            $default_school_id = isset($setting->default_school) ? $setting->default_school : 0;
            $school = $this->web->get_single('schools', array('status' => 1, 'id' => $default_school_id));
            $this->data['employees'] = $this->web->get_employee_list($school->id);
            $this->data['groups'] = get_groups();
            $this->data['list'] = TRUE;
    ...
    * ********************************************************** */
            ...
            if($_POST){
                ...
                if ($this->form_validation->run() === TRUE) {
                    ...
                    if ($insert_id) {
                        ...
                        $this->layout->view('web/admission-form');                       
                    } else {
    ...
    private function _prepare_admission_validation() {
        ...
        $this->form_validation->set_rules('document', $this->lang->line('document'), 'trim|callback_document');
    ...
    * ********************************************************** */
    public function photo() {
        if (isset($_FILES['photo']) && !empty($_FILES['photo']['name'])) {
            ...
            if (in_array(strtolower($ext), ['jpg', 'jpeg', 'png', 'gif'])) {
                ...
                $this->form_validation->set_message('photo', 'Only JPG, JPEG, PNG or GIF allowed.');
                return FALSE;
            }
        }

        // File not uploaded, but since it's optional, return TRUE
        return TRUE;
    }
    ...
    public function document() {
        if (isset($_FILES['document']) && !empty($_FILES['document']['name'])) {
            ...
            if (strtolower($ext) === 'zip') {
                ...
                $this->form_validation->set_message('document', 'Only .zip files are allowed.');
                return FALSE;
            }
        }
    ...
    private function _get_posted_admission_data() {
        ...
        if (isset($_FILES['document']) && $_FILES['document']['name']) {
            $data['document'] = $this->_upload_document();
        }
        return $data;
    }
    ...
    private function _upload_document() {
        ...
        if ($document != "") {
            ...
            if (in_array($document_type, $valid_types)) {
                ...
                $doc_path = 'tc-' . time() . '.zip';

                move_uploaded_file($_FILES['document']['tmp_name'], $destination . $doc_path);
                $return_doc = $doc_path;
            }
        }
    ...
    * ********************************************************** */
    public function contact() {
        ...
        if($this->session->userdata('front_school_id')){           
            ...
            $this->session->set_userdata('front_school_id', $school->id);
            $this->data['groups'] = get_groups();
            $opening_hours = $this->web->get_list('opening_hours', array('status' => 1, 'school_id' => $default_school_id));
            $this->data['opening_hours'] = $opening_hours;
            // need to check school subscription status
            if(!check_saas_status($school->id, 'is_enable_frontend')){                        
               redirect();
            }
