    ...
    * ********************************************************** */
    public function index() {
        ...
        if($this->session->userdata('role_id') != SUPER_ADMIN){ 
            ...
            $this->data['setting'] = $this->setting->get_single('id_card_settings', $condition);
        }  
        
        $this->data['list'] = TRUE;
        $this->layout->title($this->lang->line('manage_id_card_setting') . ' | ' . SMS);
        $this->layout->view('id_card/school', $this->data);            
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($insert_id) {
                    ...
                    redirect('card/schoolidsetting/index');
                } else {
                    error($this->lang->line('insert_failed'));
                    redirect('card/schoolidsetting/add');
                }
            } else {
                error($this->lang->line('insert_failed'));
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($updated) {
                    ...
                    redirect('card/schoolidsetting/edit/' . $this->input->post('id'));
                }
            }else{
                error($this->lang->line('update_failed'));
            } 
   ...
   {  
        ...
        if($_FILES['logo']['name']){
            ...
            if((!empty($width)) && $width > 100 || $height > 110){
                $this->form_validation->set_message('school_logo', $this->lang->line('please_check_image_dimension'));
                return FALSE;
            }
