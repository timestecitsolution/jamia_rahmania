...
class Video_Model extends MY_Model {
    ...
    public function get_video_list($school_id = null){
        
        $this->db->select('GV.*, S.school_name, G.title');
        $this->db->from('gallery_videos AS GV');
        $this->db->join('galleries AS G', 'G.id = GV.gallery_id', 'left');
        $this->db->join('schools AS S', 'S.id = GV.school_id', 'left');
         
        if($this->session->userdata('role_id') != SUPER_ADMIN){
            $this->db->where('GV.school_id', $this->session->userdata('school_id'));
        }
        if($this->session->userdata('role_id') == SUPER_ADMIN && $school_id){
            $this->db->where('GV.school_id', $school_id);
        }  
        $this->db->where('S.status', 1);
        $this->db->order_by('GV.id', 'DESC');
