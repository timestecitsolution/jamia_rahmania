...
class About extends MY_Controller {
    ...
    * ********************************************************** */
    private function _prepare_about_validation() {
        ...
        $this->form_validation->set_rules('about_title', $this->lang->line('about_school'), 'trim|required');
    ...
    * ********************************************************** */ 
    public function about_image() {
            ...
            if ($count > 4) {
                $this->form_validation->set_message('about_image', 'You can upload a maximum of 4 images.');
                return FALSE;
            }
    ...
    * ********************************************************** */
    private function _get_posted_about_data() {
        ...
        if (!empty($_FILES['about_image']['name'][0])) {
            $prev_images = explode(',', $this->input->post('prev_about_image'));
            foreach ($prev_images as $img) {
                $path = 'assets/uploads/about/' . $img;
                if (file_exists($path)) {
                    @unlink($path);
                }
            }

            $uploaded_images = $this->_upload_about_image();
            $data['about_image'] = implode(',', $uploaded_images);
        } else {
            $data['about_image'] = $this->input->post('prev_about_image');
        }
    ...
    private function _upload_about_image() {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            if ($file_name != "") {
                ...
                if (move_uploaded_file($file_tmp_name, $destination . $new_name)) {
                    $uploaded[] = $new_name;
                }
            }
        }
