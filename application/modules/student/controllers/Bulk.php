...
class Bulk extends MY_Controller {
    ...
    * ********************************************************** */
    private function _get_posted_student_dataEnglish() {
        ...
        if (($handle = fopen($destination, "r")) !== FALSE) {
            ...
            while (($arr = fgetcsv($handle)) !== false) {
                ...
                if ($arr[0] != '' && $arr[1] != '' && $arr[4] != '' && $arr[13] != '') {
                    ...
                    $guardian['name']    = isset($arr[33]) ? $arr[33] : 'Guardian';
                    $guardian['phone']   = isset($arr[34]) ? $arr[34] : '123456789';
                    ...
                    $data['gender'] = isset($arr[4]) ? $arr[4] : 'male';                                     
                    $data['blood_group'] = isset($arr[5]) ? $arr[5] : 'a_positive';
                    ...
                    $data['registration_no'] = isset($arr[14]) ? $arr[14] : 0;  
                    $data['discount_id'] = isset($arr[15]) ? $arr[15] : 0;
    ...
    private function _get_posted_student_data() {
        ...
        $this->_upload_file_excel();



        $filePath = 'assets/excel/bulk_uploaded_student.xlsx';
        ...
        foreach ($rowIterator as $row) {
            ...
            if ($arr[0] != '' && $arr[1] != '' && $arr[4] != '' && $arr[13] != '') {
                ...
                $student_identification_no = $this->student->get_list('students', [], '', '', '', 'id', 'ASC');
                $student_identification_no = count($student_identification_no) + 1;
                $student_identification_no = $last_two_digits.$this->schools[0]->id.str_pad($student_identification_no, 4, "0", STR_PAD_LEFT) ;

                $guardian_user['role_id']  = GUARDIAN;
                $guardian_user['name']     =  isset($arr[33]) ? $arr[33] : 'Guardian';
                $guardian_user['phone']    =  isset($arr[34]) ? $arr[34] : '';
                $guardian_user['email']    =  isset($arr[35]) ? $arr[35] : '';
                $guardian_user['username'] =  'p_'.$student_identification_no;
                $guardian_user['password'] =  md5(123456);
                
                $guardian['user_id'] = $this->student->create_custom_user($guardian_user);  

                $guardian['school_id']    = $_POST['school_id'];
                $guardian['name']    = isset($arr[33]) ? $arr[33] : 'Guardian';
                $guardian['phone']   = isset($arr[34]) ? $arr[34] : '123456789';
                $guardian['email']   = isset($arr[35]) ? $arr[35] : '';
                $guardian['profession']   = isset($arr[36]) ? $arr[36] : '';
                ...
                $user['phone'] = isset($arr[8]) ? $arr[8] : '';    
                $user['email'] = isset($arr[9]) ? $arr[9] : '';
                $user['username'] =  $student_identification_no;
                ...
                $data['email'] = isset($arr[9]) ? $arr[9] : '';                    
                $data['national_id'] = isset($arr[10]) ? $arr[10] : '';
                // $data['student_identification_no'] = $student_identification_no;

                if(isset($arr[11]))
                {
                    $type_info = $this->student->get_single('student_types',array('type'=>$arr[11]));
                        if(!empty($type_info))
                        {
                            $type_stu = $type_info->id;
                        }
                        else
                        {
                            $type_stu  = 19;
                ...
                $group_name = get_groups();
                $group_id = 'Nurani';
                if(isset($arr[12]))
                {

                    foreach ($group_name as $key_grp => $value_grp) {
                        if($arr[12] == $value_grp)
                        {
                            $group_id = $key_grp;
                            break;
                ...
                $enroll['student_id'] = $this->student->insert('students', $data);
                $enroll['roll_no'] = isset($arr[13]) ? $arr[13] : 1;
    ...
    * @return          : null 
    * ********************************************************** */
    private function _upload_file_excel() {

        $file = $_FILES['bulk_student']['name'];
        
        if ($file != "") {

            $destination = 'assets/excel/bulk_uploaded_student.xlsx';          
