...
class Student extends MY_Controller {
    ...
    * ********************************************************** */
    public function index($class_id = null) {
        ...
        if($school_id){
            ...
            $this->data['sections'] = $this->student->get_section($class_id, $school_id);

            $this->data['students_trash'] = $this->student->get_student_list_trash($class_id, $school_id, $school->academic_year_id);
        }
        else
        {
            if($this->session->userdata('school_id') == 0)
            {
                $school_id = 1;
                $school = $this->student->get_school_by_id($school_id);
                $this->data['students'] = $this->student->get_student_list('', $school_id, $school->academic_year_id);
                $this->data['students_trash'] = $this->student->get_student_list_trash('', $school_id, $school->academic_year_id);
            }
    ...
    {
        redirect('accounting/invoice/add/?student_id='.$id);
    }
    ...
    * ********************************************************** */
    public function add() {
        ...
        if ($_POST) {
            ...
            if($this->session->userdata('role_id') != SUPER_ADMIN){                 
                if(!check_saas_status($this->session->userdata('school_id'), 'student')){                        
        ...
        $this->data['students_trash'] = $this->student->get_student_list_trash($class_id);
        $this->data['roles'] = $this->student->get_list('roles', array('status' => 1), '', '', '', 'id', 'ASC');
        
        
        $this->data['x'] = $this->student->get_list('students', [], '', '', '', 'id', 'ASC');
        $this->data['x'] = count($this->data['x']) + 1;
    ...
    * ********************************************************** */
    ...
    public function get_single_student(){
        ...
        $this->data['logo'] = $school->logo;
        $this->data['address'] = $school->address;
    ...
    * ********************************************************** */
    public function transfer_certificate() {
        if ($_FILES['transfer_certificate']['name']) {
            ...
            list($width, $height) = getimagesize($_FILES['transfer_certificate']['tmp_name']);
            if((!empty($width)) && $width > 1200 || $height > 600){
                $this->form_validation->set_message('transfer_certificate', $this->lang->line('please_check_image_dimension'));
                return FALSE;
            }
    ...
    * ********************************************************** */
    private function _get_posted_student_data() {
        ...
        if ($this->input->post('id')) {
            ...
            $guardian_info['name']     =  $this->input->post('gud_name');
            $guardian_info['phone'] = $this->input->post('gud_phone'); 
         
            $this->student->update('guardians', $guardian_info, array('id'=>$this->input->post('guardian_id') ) );
            ...
            if($this->input->post('is_guardian') == 'exist_guardian'){
                ...
                $info['password'] = $this->input->post('gud_password') ? $this->input->post('gud_password') : 123456;
    ...
    * ********************************************************** */
    ...
    public function delete_restore($id = null) {
        ...
        if ($x == true) {
            ...
            success("Data Restore Successfully");
        } else {
            error($this->lang->line('delete_failed'));
        }
