<section class="content invoice profile_img layout-box">
      ...
      <div class="row invoice-info">          
          ...
          <div class="col-md-6 col-sm-6 col-xs-6 invoice-col">             
                <strong><?php echo $this->lang->line('invoice'); ?>: </strong> #<?php echo $donar->custom_invoice_id; ?>                                                   
                <br><strong><?php echo $this->lang->line('status'); ?>:</strong> <?php echo get_paid_status($donar->paid_status); ?>
                 <br><strong><?php echo $this->lang->line('donation_cycle'); ?>:</strong> <?php 
                    $cycles = explode(',', $donar->donation_cycle); 
                    $labels = array_map('get_donation_cycle_label', $cycles);
                    echo implode(', ', $labels);
                 ?>
                <br><strong><?php echo $this->lang->line('date'); ?>:</strong> <?php echo date($this->global_setting->date_format, strtotime($donar->date)); ?>
          </div>
      </div>   
                    ...
                    <table class="table">
                        <tbody>
                            <tr>
                                ...
                                <td><?php echo $settings->currency_symbol; ?><?php echo $donar->gross_amount; ?></td>
                            </tr>
                            <?php if($donar->paid_status == 'paid'){ ?>
                                <tr>
                                    <th><?php echo $this->lang->line('paid_date'); ?>:</th>
                                    <td><?php echo date($this->global_setting->date_format, strtotime($donar->date)); ?></td>
                                </tr>
                            <?php } ?>
                            <tr>
                                <th style="width:56%"><?php echo $this->lang->line('collector'); ?>:</th>
                                <td><?php echo $donar->collector; ?></td>
                            </tr>
                            <tr>
...
<section>
    <div class="row no-print">
        <div class="col-xs-12">
            ...
            <?php if($donar->paid_status != 'paid'){ ?>
                <a href="<?php echo site_url('accounting/payment/index/'.$donar->inv_id); ?>"><button class="btn btn-success pull-right"><i class="fa fa-credit-card"></i> <?php echo $this->lang->line('pay_now'); ?></button></a>
            <?php } ?>
        </div>
    </div>
</section>