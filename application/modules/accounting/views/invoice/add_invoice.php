                            ...
                            <div class="x_content"> 
                               ...
                               ?>
                                ...
                                <div class="item form-group">
                                    ...
                                    <div class="col-md-6 col-sm-6 col-xs-12">
                                        <input  class="form-control col-md-7 col-xs-12" readonly="readonly"  name="net_amount"  id="net_amount" value="<?php echo isset($post['amount']) ?  ($post['amount']-$post['discount']) : '0.00'; ?>" placeholder="Net Amount" required="required" type="text" autocomplete="off">
                                        <div class="help-block"><?php echo form_error('amount'); ?></div>
                                    </div>
                                </div> 
                                ...
                                <div class="item form-group" id="zakat_eligibility_container" style="display:none;">
                                    ...
                                    <div class="col-md-6 col-sm-6 col-xs-12">
                                        <div class="form-check form-switch">
                                            <input class="form-check-input" id="zakat_eligibility" name="zakat_eligibility" type="checkbox" role="switch" value="1" > <span>Zakat fund amount: <?php echo $zakat_fund_amount;?></span>
                                            <?php //if ($is_disabled): ?>
...
<script type="text/javascript"> 
    ...
    function student_id_change_new(student_id,income_head_id)
    {
        ...
        $.ajax({       
            type   : "POST",
            url    : "<?php echo site_url('accounting/invoice/get_month_by_student'); ?>",
            data   : {school_id:school_id, student_id : student_id, income_head_id : income_head_id},               
            async  : false,
            success: function(response){    
                const res = JSON.parse(response);   
                if(res.html) {
    ...
    $(document).ready(function () {
            $(document).on('change', '.fn_income_head_id', function () {
                var is77Checked = $('#income_head_id_77').is(':checked');
                var is74Checked = $('#income_head_id_74').is(':checked');
                if(is77Checked){
                    partial_payment_hide_and_toggle_77(is77Checked);
                }else if(is74Checked){
                    partial_payment_hide_and_toggle_74(is74Checked);
                }
                calculate_net_amount();
            });

            var is77CheckedOnLoad = $('#income_head_id_77').is(':checked');
            var is74CheckedOnLoad = $('#income_head_id_74').is(':checked');

            if(is77CheckedOnLoad){
                partial_payment_hide_and_toggle_77(is77CheckedOnLoad);
            }else if(is74CheckedOnLoad){
                partial_payment_hide_and_toggle_74(is74CheckedOnLoad);
            }
        ...
        function partial_payment_hide_and_toggle_77(is77Checked) {
            if (is77Checked) {
                // $('#paid_status option[value="partially_paid"]').remove();
                $('#income_head_id_74').prop('checked', false);
                if ($('#paid_status').val() === 'partially_paid') {
                    $('#paid_status').val('').trigger('change.select2');
                }

                $('.fn_income_head_id').each(function () {
                    if ($(this).val() != 77) {
                ...
                if ($('#paid_status option[value="partially_paid"]').length === 0) {
                    var partialText = "<?php echo $this->lang->line('partial_payment'); ?>";
                    $('#paid_status').append('<option value="partially_paid">' + partialText + '</option>');
                }

                $('.fn_income_head_id').prop('disabled', false);
                $('#zakat_eligibility_container').hide();
            }

            $('#paid_status').trigger('change.select2');
        }

        function partial_payment_hide_and_toggle_74(is74Checked) {
            if (is74Checked) {
                $('#income_head_id_77').prop('checked', false);
                ...
                $('.fn_income_head_id').prop('disabled', false).prop('checked', true);
                $('#zakat_eligibility_container').hide();
            }
  ...
  // single  
   function get_single_fee_type_by_school(school_id){
    ...
    function get_single_fee_amount(income_head_id){
        ...
        student_id_change_new(student_id,income_head_id);
        $.ajax({       
            type   : "POST",
