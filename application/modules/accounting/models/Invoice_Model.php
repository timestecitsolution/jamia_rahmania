...
class Invoice_Model extends MY_Model {
    ...
    public function get_invoice_list($school_id = null, $due = null, $academic_year_id = null){
        ...
        $this->db->select('I.*, SC.school_name,  S.name AS student_name,S.phone AS phone_number, AY.session_year, C.name AS class_name');
        $this->db->from('invoices AS I');        
        $this->db->join('classes AS C', 'C.id = I.class_id', 'left');
    ...
    public function get_total_due_by_student($user_id) {
    ...
    public function update_latest_due_amount($user_id) {
        $this->db->select('id');
        ...
        if ($query->num_rows() > 0) {
            $latest_invoice_id = $query->row()->id;
    
            $this->db->where('id', $latest_invoice_id);
            $this->db->update('invoices', ['due_amount' => '0']);
    ...
    public function get_student_list( $school_id, $academic_year_id, $class_id, $student_id = null, $status_type = null){
        ...
        // $this->db->select('E.roll_no, S.discount_id,  S.id, S.user_id, S.name, S.admission_no, S.student_identification_no, S.is_hostel_member, S.is_transport_member');
        $this->db->select('E.roll_no, S.discount_id, S.school_student_id ,  S.id, S.user_id, S.name, S.admission_no, S.is_hostel_member, S.is_transport_member');
