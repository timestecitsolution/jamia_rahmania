...
class Expenditure extends MY_Controller {
    ...
    public function index($school_id = null) {
        ...
        if($this->session->userdata('role_id') != SUPER_ADMIN){
            ...
            $this->data['income_heads'] = $this->expenditure->get_list('income_heads', $condition);  
    ...
    public function add() {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($insert_id) {
                    if($data['income_head_id'] == '76'){
                        $construction_fund_prev_amount = construction_fund();
                        $updated_construction_fund = $construction_fund_prev_amount->construction_fund - $data['amount'];
                        $this->expenditure->update('global_setting', array('construction_fund' => $updated_construction_fund), array('id' => 1));
                    }elseif($data['income_head_id'] == '75'){
                        $general_fund_prev_amount = general_fund();
                        $updated_general_fund = $general_fund_prev_amount->general_fund - $data['amount'];
                        $this->expenditure->update('global_setting', array('general_fund' => $updated_general_fund), array('id' => 1));
                    }elseif($data['income_head_id'] == '77'){
                        $boarding_fund_prev_amount = boarding_fund();
                        $updated_boarding_fund = $boarding_fund_prev_amount->boarding_fund - $data['amount'];
                        $this->expenditure->update('global_setting', array('boarding_fund' => $updated_boarding_fund), array('id' => 1));
                    }elseif($data['income_head_id'] == '67'){
                        $zakat_fund_prev_amount = zakat_fund();
                        $updated_zakat_fund = $zakat_fund_prev_amount->zakat_fund - $data['amount'];
                        $this->expenditure->update('global_setting', array('zakat_fund' => $updated_zakat_fund), array('id' => 1));
                    }
    ...
    public function edit($id = null) {  
       ...
       check_permission(EDIT);
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                $data = $this->_get_posted_expenditure_data();
                $expenditure = $this->expenditure->get_single('expenditures', array('id' => $this->input->post('id')));
                $add_amount =  $data['amount'] - $expenditure->amount;
                $updated = $this->expenditure->update('expenditures', $data, array('id' => $this->input->post('id')));

                if ($updated) {
                    if($data['income_head_id'] == '76'){
                        $construction_fund_prev_amount = construction_fund();
                        $updated_construction_fund = $construction_fund_prev_amount->construction_fund - $add_amount;
                        $this->expenditure->update('global_setting', array('construction_fund' => $updated_construction_fund), array('id' => 1));
                    }elseif($data['income_head_id'] == '75'){
                        $general_fund_prev_amount = general_fund();
                        $updated_general_fund = $general_fund_prev_amount->general_fund - $add_amount;
                        $this->expenditure->update('global_setting', array('general_fund' => $updated_general_fund), array('id' => 1));
                    }elseif($data['income_head_id'] == '77'){
                        $boarding_fund_prev_amount = boarding_fund();
                        $updated_boarding_fund = $boarding_fund_prev_amount->boarding_fund - $add_amount;
                        $this->expenditure->update('global_setting', array('boarding_fund' => $updated_boarding_fund), array('id' => 1));
                    }elseif($data['income_head_id'] == '67'){
                        $zakat_fund_prev_amount = zakat_fund();
                        $updated_zakat_fund = $zakat_fund_prev_amount->zakat_fund - $add_amount;
                        $this->expenditure->update('global_setting', array('zakat_fund' => $updated_zakat_fund), array('id' => 1));
                    }
    ...
    // public function validate_expenditure_amount($amount)
    // {
    //     $income_head_id = $this->input->post('income_head_id');
    //     if($income_head_id == '75'){
    //         $general_fund = general_fund();
    //         $fund = $general_fund->general_fund;
    //     }elseif($income_head_id == '76'){
    //         $construction_fund = construction_fund();
    //         $fund = $construction_fund->construction_fund;
    //     }elseif($income_head_id == '77'){
    //         $boarding_fund = boarding_fund();
    //         $fund = $boarding_fund->boarding_fund;
    //     }

    //     if ($fund < $amount) {
    //         $this->form_validation->set_message('validate_expenditure_amount', 'You dont have sufficient fund');
    //         return false;
    //     }
    ...
    public function get_single_expenditure(){
       ...
       $school_id = $this->data['expenditure']->school_id;      
       $this->data['settings']   = $this->expenditure->get_school_by_id($school_id);
    ...
    public function delete($id = null) {
        ...
        if ($this->expenditure->delete('expenditures', array('id' => $id))) {
            ...
            if($expenditure->income_head_id == '75'){
                $general_fund_prev_amount = general_fund();
                $updated_general_fund = $general_fund_prev_amount->general_fund + $expenditure->amount;
                $this->expenditure->update('global_setting', array('general_fund' => $updated_general_fund), array('id' => 1));
            }elseif($expenditure->income_head_id == '76'){
                $construction_fund_prev_amount = construction_fund();
                $updated_construction_fund = $construction_fund_prev_amount->construction_fund + $expenditure->amount;
                $this->expenditure->update('global_setting', array('construction_fund' => $updated_construction_fund), array('id' => 1));
            }elseif($expenditure->income_head_id == '77'){
                $boarding_fund_prev_amount = boarding_fund();
                $updated_boarding_fund = $boarding_fund_prev_amount->boarding_fund + $expenditure->amount;
                $this->expenditure->update('global_setting', array('boarding_fund' => $updated_boarding_fund), array('id' => 1));
            }elseif($expenditure->income_head_id == '67'){
                $zakat_fund_prev_amount = zakat_fund();
                $updated_zakat_fund = $zakat_fund_prev_amount->zakat_fund + $expenditure->amount;
                $this->expenditure->update('global_setting', array('zakat_fund' => $updated_zakat_fund), array('id' => 1));
            }
    ...
    public function get_income_head_by_school() {
        ...
        $income_heads = $this->expenditure->get_list('income_heads', array('school_id'=>$school_id));  
