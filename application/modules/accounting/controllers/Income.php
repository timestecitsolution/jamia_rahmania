...
class Income extends MY_Controller {
    ...
    public function get_single_income(){
       ...
       $school_id = $this->data['income']->school_id;      
       $this->data['settings']   = $this->income->get_school_by_id($school_id);
    ...
    public function delete($id = null) {
        ...
        if ($this->income->delete('invoices', array('id' => $id))) { 
            ...
            $invoice_details = $this->income->get_single('invoice_detail', array('invoice_id' => $id));
            if($invoice_details->income_head_id == '75'){
                $general_fund_prev_amount = general_fund();
                $updated_general_fund = $general_fund_prev_amount->general_fund - $invoice_details->net_amount;
                $this->income->update('global_setting', array('general_fund' => $updated_general_fund), array('id' => 1));
            }
            if($invoice_details->income_head_id == '76'){
                $construction_fund_prev_amount = construction_fund();
                $updated_construction_fund = $construction_fund_prev_amount->construction_fund - $invoice_details->net_amount;
                $this->income->update('global_setting', array('construction_fund' => $updated_construction_fund), array('id' => 1));
            }
    ...
    private function _save_invoice_detail($invoice_id){
        ...
        if ($this->input->post('id')) {
            ...
            $invoice_details = $this->income->get_single('invoice_detail', array('invoice_id' => $invoice_id));
            $diff_amount = $inv_detail['net_amount'] - $invoice_details->net_amount;

                if(($inv_detail['income_head_id'] == '75')){
                    $general_fund_prev_amount = general_fund();
                    $updated_general_fund = $general_fund_prev_amount->general_fund + $diff_amount;
                    $this->income->update('global_setting', array('general_fund' => $updated_general_fund), array('id' => 1));
                }
                if(($inv_detail['income_head_id'] == '76')){
                    $construction_fund_prev_amount = construction_fund();
                    $updated_construction_fund = $construction_fund_prev_amount->construction_fund + $diff_amount;
                    $this->income->update('global_setting', array('construction_fund' => $updated_construction_fund), array('id' => 1));
                }
            ...
            if(($inv_detail['income_head_id'] == '75')){
                $general_fund_prev_amount = general_fund();
                $updated_general_fund = $general_fund_prev_amount->general_fund + $inv_detail['net_amount'];
                $this->income->update('global_setting', array('general_fund' => $updated_general_fund), array('id' => 1));
            }
            if(($inv_detail['income_head_id'] == '76')){
                $construction_fund_prev_amount = construction_fund();
                $updated_construction_fund = $construction_fund_prev_amount->construction_fund + $inv_detail['net_amount'];
                $this->income->update('global_setting', array('construction_fund' => $updated_construction_fund), array('id' => 1));
            }
        }
