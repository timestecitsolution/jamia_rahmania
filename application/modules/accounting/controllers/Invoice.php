...
class Invoice extends MY_Controller {
    ...
    * ********************************************************** */
    public function add($school_id = null) {
        ...
        if(!empty($student_id))
        {
            $enrollment = $this->db->order_by('id', 'DESC')->get_where('enrollments', ['student_id' => $student_id], 1)->row();

            $this->data['auto_class_id'] = $enrollment->class_id;
            $this->data['auto_student_id'] = $enrollment->student_id;
        }
        ...
        $this->layout->view('invoice/add_invoice', $this->data);
    }
    ...
    * ********************************************************** */
    private function _prepare_invoice_validation() {
        ...
        $this->form_validation->set_rules('amount_recieved_from_zakat', $this->lang->line('amount_recieved_from_zakat'), 'trim'); 
    ...
    // public function check_zakat_limit($received_zakat)
    // {
    //     if ($received_zakat === '' || $received_zakat === null) {
    //         return TRUE;
    //     }

    //     $zakat_fund_data = zakat_fund(); 
    //     $available_zakat = isset($zakat_fund_data->zakat_fund) ? $zakat_fund_data->zakat_fund : 0;

    //     if ((float)$received_zakat > (float)$available_zakat) {
    //         $this->form_validation->set_message('check_zakat_limit', 'Received zakat amount cannot exceed the available zakat fund.');
    //         return FALSE;
    //     }
    ...
    private function _get_posted_invoice_data() {
        ...
        $items[] = 'zakat_eligibility';
        $items[] = 'amount_recieved_from_zakat';  
        $items[] = 'month';        
        $items[] = 'paid_status';        
        $items[] = 'partial_payment_amount';
        $items[] = 'total_payable';
        $items[] = 'due_amount';
        $items[] = 'note';
        $items[] = 'discount';
        ...
        if(!empty($data["month"][0]))
        {
            ...
            foreach ($income_head_id as $key_income_head_id => $value_income_head_id) {
                ...
                foreach ($data["month"] as $key => $month_id) 
                {
                    ...
                    $x_month_name = $string = str_replace(' ', '', $monthName);
                    $this->db->select('I.*');
                    $this->db->from('invoices AS I');
                    $this->db->join('invoice_detail AS E', 'E.invoice_id = I.id', 'left');
                    $this->db->where('I.school_id', $data['school_id']);
                    $this->db->where('E.income_head_id', $value_income_head_id);
                    $this->db->where('I.paid_status', 'unpaid');
                    $this->db->like('I.month', $x_month_name);
                    $this->db->where('I.user_id', $student->user_id);
                    $invoice_old = $this->db->get()->result(); 
                    if(!empty($invoice_old))
                    {
                        foreach ($invoice_old as $key => $value) {
                            
                            $invoice = $this->invoice->get_single('invoices', array('id' => $value->id));
                            if ($this->invoice->delete('invoices', array('id' => $value->id))) {  
                                $this->invoice->delete('invoice_detail', array('invoice_id' => $value->id));
                                $this->invoice->delete('transactions', array('invoice_id' => $value->id));
        ...
        // save invoice data
         ...
         if($inv_data['paid_status'] == 'unpaid'){
            $inv_data['due_amount'] = 0;
         }
         if(($inv_data['paid_status'] == 'partially_paid') || ($inv_data['paid_status'] == 'paid')){
            $update_invoice = $this->invoice->update_latest_due_amount($student->user_id);
         }
         $zakat_fund_prev_amount = zakat_fund();
         $invoice_id = $this->invoice->insert('invoices', $inv_data);
         if(($invoice_id) && ($inv_data['zakat_eligibility'] != null)){
            $zakat_fund_deduction = array(
                'zakat_fund' => $data['amount_recieved_from_zakat']
            );

            $updated_zakat_fund = $zakat_fund_prev_amount->zakat_fund - $zakat_fund_deduction['zakat_fund'];
            $this->invoice->update('global_setting', array('zakat_fund' => $updated_zakat_fund), array('id' => 1));
         }
        ...
        foreach ($this->input->post('income_head_id') as $key=>$value){
            ...
            if(($invoice_id) && ($inv_detail['income_head_id'] == '77')){
                if($data['paid_status'] == 'partially_paid'){
                    $data['total_payable'] = $data['partial_payment_amount'];
                }
                
                $updated_boarding_fund = $boarding_fund_prev_amount->boarding_fund + $data['total_payable'];
                $this->invoice->update('global_setting', array('boarding_fund' => $updated_boarding_fund), array('id' => 1));
            }
            if($inv_detail['income_head_id'] == '74' || $inv_detail['income_head_id'] == '773'){
                if($data['paid_status'] == 'partially_paid'){
                    $data['total_payable'] = $data['partial_payment_amount'];
                }

                $general_fund_prev_amount = general_fund();
                $updated_general_fund = $general_fund_prev_amount->general_fund + $data['total_payable'];
                $this->invoice->update('global_setting', array('general_fund' => $updated_general_fund), array('id' => 1));
            }
        ...
        $this->_prepare_accounts_validation($data);
    ...
    private function _prepare_accounts_validation($data)
    {
        
        $posted_data['validation_type'] = 'invoice';
        $posted_data['transaction_id'] = $data['transaction_id'];
        $posted_data['transaction_details'] = $data['custom_invoice_id'] . (!empty($data['note']) ? " | Note - " . $data['note'] : '');
        $posted_data['transaction_amount'] = $data['total_payable'] ;
        $posted_data['transaction_date'] = $data['date'] ;
        cash_book_validation($posted_data);
    }


    public function get_latest_due_amount() {
        ...
        $total_due = $this->invoice->get_total_due_by_student($user_id);
    
        echo json_encode(['status' => 'success', 'due_amount' => $total_due]);
    }
    ...
    * ********************************************************** */
    public function delete($id = null) {
        ...
        $invoice_details = $this->invoice->get_single('invoice_detail', array('invoice_id' => $id));
        if($invoice->amount_recieved_from_zakat != null || $invoice->amount_recieved_from_zakat == 0){
            $zakat_fund_prev_amount = zakat_fund();
            if($invoice->paid_status == 'partially_paid'){
                    $invoice->total_payable = $invoice->partial_payment_amount;
                }
            $updated_zakat_fund = $zakat_fund_prev_amount->zakat_fund + $invoice->amount_recieved_from_zakat;
            $this->invoice->update('global_setting', array('zakat_fund' => $updated_zakat_fund), array('id' => 1));
        }
        if($invoice_details->income_head_id == '77'){
            $boarding_fund_prev_amount = boarding_fund();
            if($invoice->paid_status == 'partially_paid'){
                    $invoice->total_payable = $invoice->partial_payment_amount;
                }
            $updated_boarding_fund = $boarding_fund_prev_amount->boarding_fund - $invoice->total_payable;
            $this->invoice->update('global_setting', array('boarding_fund' => $updated_boarding_fund), array('id' => 1));
        }
        if($invoice_details->income_head_id == '74' || $invoice_details->income_head_id == '773'){
                $general_fund_prev_amount = general_fund();
                if($invoice->paid_status == 'partially_paid'){
                    $invoice->total_payable = $invoice->partial_payment_amount;
                }
                $updated_general_fund = $general_fund_prev_amount->general_fund - $invoice->total_payable;
                $this->invoice->update('global_setting', array('general_fund' => $updated_general_fund), array('id' => 1));
        }
    ...
    private function _save_transaction($data){
        if($data['paid_status'] == 'paid' || $data['paid_status'] == 'partially_paid'){
            $txn = array();
            $txn['school_id'] = $data['school_id']; 

            $zakat_fund_prev_amount = zakat_fund();
            if(($data['zakat_eligibility'] == 1) && (($zakat_fund_prev_amount->zakat_fund != 0) || ($zakat_fund_prev_amount->zakat_fund != null))){
                $txn['amount'] = $data['partial_payment_amount'];
            }else{
            ...
            {            
                ...
                else
                {
                    ...
                    foreach (json_decode($data['month']) as $month_key => $month_value) {
                        ...
                        $monthArray = json_decode($monthData, true);
                        
                       
                        if($month_key+1 != sizeof($monthArray) )
                        {
                            $month .= $monthName.',';
                        }
                        else
    ...
    public function get_single_fee_type_by_school(){
        ...
        if (!empty($income_heads)) {
            foreach ($income_heads as $obj) { 
                ...
                $str .= '<input onclick="get_single_fee_amount('.$obj->id.')" type="checkbox" name="income_head_id['.$obj->id.']" id="income_head_id_'.$obj->id.'" class="fn_income_head_id" value="'.$obj->id.'" > '.$obj->title.'<br/>';
            }
        }
    ...
    public function get_month_by_student() {
        ...
        if(!empty($income_head_id))
        {
            ...
            ->from('invoices')
            ->join('invoice_detail', 'invoice_detail.invoice_id = invoices.id', 'left')
            ->where('invoices.user_id', $student->user_id)
            ->where('invoices.academic_year_id', $school->academic_year_id)
            ->where('invoices.invoice_type', 'invoice')
            ->where_in('invoices.paid_status', ['paid', 'partially_paid'])
            ->where('invoice_detail.income_head_id', $income_head_id)
            ->get()
            ->result_array();
        }
        else
        {
            ...
            ->where_in('paid_status', ['paid', 'partially_paid'])
            ->get()
            ->result_array();
        }
        ...
        foreach ($invoices as $invoices_value) {
            $month_value = json_decode($invoices_value["month"]);
        ...
        echo json_encode([
            'html' => $str,
            'zakat_eligible' => isset($student->zakat_eligable) ? $student->zakat_eligable : 0
        ]);
    }

    public function check_coaching_fee_enable()
    {
        
        $student_id = $this->input->post('student_id');
        $student_info = $this->invoice->get_single('students', array('id' => $student_id));
        
        if ($student_info->coaching == 1) {
            echo 1;
        }
        else
        {
    ...
    public function get_month_info()
    {
        ...
        {
            if($income_head->collection_type == "Monthly")
            {
                echo 1;
            }
            else
    ...
    public function get_all_fee_amount()
    {
        $school_id = $this->input->post('school_id');   
        $student_id = $this->input->post('student_id');   
        $class_id = $this->input->post('class_id');   
        ...
        $amount = 0;
        foreach ($income_head_id as $key => $value) {
           $income_head = $this->invoice->get_single('income_heads', array('id' => $value, 'school_id'=>$school_id));

            if(!empty($income_head))
            {
                $fees_amount = $this->invoice->get_single('fees_amount', array('income_head_id' => $value, 'school_id'=>$school_id, 'class_id' => $class_id));
                if($income_head->collection_type == "Monthly")
                {
                    $amount += $fees_amount->fee_amount * $number_of_month;
                }
                else
                {
                    $amount += $fees_amount->fee_amount;
                }
            }
    ...
    public function get_bulk_fee_amount(){
        ...
        if(!empty($students) && $head_ids != ''){            
            ...
            foreach($students as $obj){                
                ...
                $student_str .= '<div class="multi-check"><input type="checkbox" class="studentchk" name="students['.$obj->id.']" value="'.$amount.'" /> '.$obj->name.' [ID-'.$obj->school_student_id.']['.$school->currency_symbol.$amount.']</div>';
    ...
    public function get_student_by_class() {
        ...
        if (!empty($students)) {
            foreach ($students as $obj) {
                ...
                $str .= '<option data-discount="' . $obj->discount_id . '" value="' . $obj->id . '" ' . $selected . '>' . $obj->name . ' [ Roll -' . $obj->roll_no . ', Admission No -' . $obj->admission_no . ',' . ' User ID -' . $obj->student_identification_no . ']</option>';
            }
        }
    ...
    public function get_discount_type()
    {
        ...
        $query = $this->db->get_where('discounts', array('id' => $discountId))->row();
