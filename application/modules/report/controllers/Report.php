...
class Report extends My_Controller {
    ...
    public function __construct() {
        ...
        if($this->session->userdata('role_id') != SUPER_ADMIN){                 
            if(!check_saas_status($this->session->userdata('school_id'), 'is_enable_report')){                        
    ...
    * ********************************************************** */
    public function duefee() {
        ...
        if ($_POST) {
            ...
            $user_id = '';
            if($student_id){
                $student = $this->report->get_single('students', array('id' => $student_id)); 
                $user_id = $student->user_id;
            }

            $this->data['sbalance'] = $this->report->get_student_due_fee_report($school_id, $academic_year_id, $class_id, $user_id,$date_from,$date_to,$income_head);
        ...
        $this->data['income_heads'] = $this->db->get('income_heads')->result_array();
    ...
    * ********************************************************** */
    public function feecollection() {
        ...
        if ($_POST) {
            ...
            $this->data['feecollection'] = $this->report->get_student_fee_collection_report($school_id, $academic_year_id, $class_id, $user_id, $date_from, $date_to,$income_head);
        }
    ...
    * @description     : Load donation report user interface                 
    *                    with various filtering options
    *                    and process to load donation report   
    * @param           : null
    * @return          : null 
    * ********************************************************** */
    public function donation() {
        ...
        if ($_POST) {
            ...
            $this->data['school_id'] = $school_id;
            $this->data['academic_year_id'] = $academic_year_id;
            $this->data['doner_type'] = $doner_type;
            $this->data['donation_category'] = $donation_category;
            $this->data['date_from'] = $date_from;
            $this->data['date_to'] = $date_to;     
            ...
            $this->data['donation_lists'] = $this->report->get_donation_list($school_id, $academic_year_id, $doner_type, $donation_category,$date_from, $date_to);
        ...
        $this->data['report_url'] = site_url('report/donation');
        $this->layout->title($this->lang->line('donation_report') . ' | ' . SMS);
    ...
    * ********************************************************** */
    public function sinvoice() {
        ...
        if ($_POST) {
            ...
            $this->data['sbalance'] = $this->report->get_student_invoice_report($school_id, $academic_year_id, $class_id, $user_id);
        ...
        for ($i = 0; $i < $days; $i++) {           
            ...
            if(!empty($income)){
                ...
                foreach($income as $inc){
                    $data[$j+1]['head'] = $inc->invoice_type;                       
                    $data[$j+1]['debit'] = 0;                       
    ...
    * ********************************************************** */
    ...
    public function tabulation() {
        ...
        if ($_POST) {
            ...
            $this->data['exams'] = $this->report->get_list('exams', $condition, '', '', '', 'id', 'ASC');
            $this->data['subjects'] = $this->db->get_where('subjects',array('class_id' => $class_id, 'school_id' => $school_id, 'is_group' => 0))->result();
            if(!empty($group_id))
            {
                $this->data['group_subjects'] = $this->db->get_where('subjects',array('class_id' => $class_id, 'school_id' => $school_id, 'is_group' => 1, 'group_id' => $group_id))->result();
            }
            $this->data['school'] = $this->report->get_school_by_id($school_id);
            $this->data['class'] = $this->db->get_where('classes', array('id'=>$class_id, 'school_id'=>$school_id))->row()->name;
            if($section_id){
            ...
            if(!empty($group_id))
            {
                $this->data['examresult'] = $this->report->get_student_examresult_report_new($school_id, $academic_year_id, $class_id, $section_id, $group_id);
            }
            else
            {
                $this->data['examresult'] = $this->report->get_student_examresult_report_new($school_id, $academic_year_id, $class_id, $section_id);
            }
        ...
        $this->data['report_url'] = site_url('report/tabulation');
        $this->layout->title($this->lang->line('tabulation_sheet') . ' | ' . SMS);
        $this->layout->view('student/tabulation', $this->data);
