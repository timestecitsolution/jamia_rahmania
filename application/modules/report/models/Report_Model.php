...
class Report_Model extends MY_Model {
    ...
    public function get_donation_list($school_id, $academic_year_id, $doner_type = null, $donation_category = null, $date_from = null, $date_to = null)
    {
        ...
        if (!empty($doner_type)) {
            $this->db->where('D.doner_type', $doner_type);
        }
    
        if (!empty($donation_category)) {
            $this->db->where('D.donation_category', $donation_category);
        }
    ...
    public function get_student_due_fee_report($school_id, $academic_year_id, $class_id, $user_id,$date_from,$date_to,$income_head = null){
        
        // var_dump($date_to);
        // exit();
        $this->db->select('I.*, SUM(T.amount) AS paid_amount, C.name AS class_name, ST.name AS student,IH.title AS fee_type,  AY.session_year');
        $this->db->from('invoices AS I');   
        $this->db->join('transactions AS T', 'T.invoice_id = I.id', 'left');
        ...
        if($income_head){
            $this->db->where('ID.income_head_id', $income_head);
        } 
    ...
    public function get_student_fee_collection_report($school_id, $academic_year_id, $class_id, $user_id, $date_from, $date_to,$income_head = null){
        
        $this->db->select('T.*, T.note,ST.name AS student, ST.school_student_id student_id, C.name AS class_name, IH.title AS fee_type, I.id AS inv_id, AY.session_year,I.month as month, I.custom_invoice_id AS custom_invoice_id');
    ...
    //         $final_query = "$expenditure_query ORDER BY payment_date ASC";
    //     } else {
    ...
    public function get_student_examresult_report($school_id, $academic_year_id, $class_id, $section_id){
        ...
        $this->db->select('FR.*, G.name AS grade, E.roll_no, ST.name AS student,ST.id AS student_id ,C.name AS class_name, S.name AS section, AY.session_year');
    ...
    public function get_student_examresult_report_new($school_id, $academic_year_id, $class_id, $section_id, $group_id = null){
        ...
        if($group_id){
            $this->db->where('ST.group', $group_id);
        } 
